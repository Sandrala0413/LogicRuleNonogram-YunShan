...
void insertHash(
    const Clue& problem, const uint64_t& nowString,
    const uint64_t& settleString) // nowString = line, settleString = newLine
{
...
bool findHash(const Clue& problem, const uint64_t& nowString,
	...
	uint64_t key = problem.hash ^ nowString; // 計算哈希鍵

	key %= HTABLE_SIZE; // 因為 "static hashNode hashTable[HTABLE_SIZE];"
	                    // 所以需要將鍵限制在表的範圍內

	if (memcmp(&hashTable[key].nowString, &nowString, sizeof(uint64_t)) != 0)
		return false; // 如果當前的 nowString 與哈希表中的不相同，返回 false
	if (memcmp(&hashTable[key].lineProblem, &problem, sizeof(problem)) != 0)
		return false; // 如果當前的 problem 與哈希表中的不相同，返回 false
