...
void listPixel(FullyProbe& fp, Board& b) {
	...
	Dual_for(i, j) if (getBit(b, i, j) == BIT_UNKNOWN) {
		node tmp;
		tmp.x = i;
		tmp.y = j;
		unk.push_back(tmp);
	...
	printf("[xxxxxx]        ");
	Dual_for(i, j) if (getBit(b, i, j) == BIT_UNKNOWN) printf("%4zu ", i);
	puts("");

	printf("[xxxxxx]        ");
	Dual_for(i, j) if (getBit(b, i, j) == BIT_UNKNOWN) printf("%4zu ", j);
	puts("");
	...
	for (size_t i = 0; i < unk.size(); ++i) {
		...
		for (int v = 0; v < 2; ++v) {
			...
			for (size_t k = 0; k < unk.size(); ++k) {
				...
				int y = unk[k].y;
				printf("    %c",
				       getBit(gp, x, y) == BIT_ZERO
				           ? '0'
				           : (getBit(gp, x, y) == BIT_ONE ? '1' : 'u'));
			}
			putchar('\n');
		}
	}
...
void expandInputFile(const char* name) {
	...
	for (int i = 0; i < 1000; ++i) {
		int prob = i % bound;
		fout << "$" << i + 1 << endl;
		for (int j = 0; j < 50; ++j) fout << input[prob * 50 + j] << endl;
	}
}
